...
Board.prototype.gameOverSolve = function(row,col){
	...
	console.log("inside game over solve now");
	...
	for(var i =1; i<=this.rows; i++){
		...
		for(var j =1; j<= this.cols; j++){
			console.log("inside game over solve outer loop");
...
console.log("game over solve inside first loop");
	...
	if(this.field[i][j].isFlag()){
		...
		console.log("game over solve inside second loop");
	}
...
console.log("The status of this field is "+ this.field[coord.row][coord.col].status);
if(this.field[coord.row][coord.col].isOpen()){
	console.log("this field is already open");
	return false;
}else
{
